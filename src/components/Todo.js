...
function Todo() { 
  ...
  return (
    <>
        ...
        <button type="button" className="btn btn-secondary" onClick={() => dispatch(addTask(inputTask, inputDesc), setInputTask(''),setInputDesc(''), alert("Task Added Successfully"))}>
        ...
        <i className="fa-solid fa-trash-can fa-large my-2" title="Delete Item" onClick={()=>{dispatch(deleteTask(elem.id))}}></i>
        </div>
        )  
        })
        }
